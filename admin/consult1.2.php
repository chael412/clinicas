...
include ('modal/modal-consultEmployees2.php');
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h5 class="m-0 font-weight-bold text-brown">Employees Monthly Consultation
                <button type="button" class="d-none d-sm-inline-block float-right btn btn-sm btn-success shadow-sm m-1"
                    data-toggle="modal" data-target="#modal_consultADD12"><i class="fas fa-plus mx-1"></i>Create
                    Consult</button>
            </h5>
        </div>
        <div class="card-body">
            ...
            <?php
            $query = "SELECT ctm.ctm_id, us.u_id, CONCAT(us.firstname, ' ', us.middlename,' ', us.lastname) AS user_fullname, ctm.chief_complaints, ctm.recommendation, ctm.process_date, GROUP_CONCAT(m.medicine_name SEPARATOR ', ') AS medicines
            FROM consult_monthly ctm
            INNER JOIN consult_monthly_medicine ctmm ON ctm.ctm_id = ctmm.ctmm_id
            LEFT JOIN medicine m ON ctmm.mdn_id = m.mdn_id
            INNER JOIN users us ON ctm.u_id = us.u_id
            INNER JOIN employees emp ON us.u_id = emp.u_id
            GROUP BY  us.u_id
            ORDER BY ctm.ctm_id DESC           
            ";
                                    ...
                                    <td>
                                        <div class="row justify-content-center">
                                            <div class="col col-lg-2">
                                                <form action="consult_view12.php" method="POST">
